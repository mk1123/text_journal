...
list_of_messages = ['Hey! How are you doing today?',
                    'Greetings! Do you have a moment to talk about your day?', 'Hi! Tell me a pertinent fact about your day!']
can
...
def update2():
    ...
    for user in active_users:
        ...
        # print(client_string)
        # print(user.name)
        user_exists = db.engine.execute(
            "select date from entries where name='" + user.name + "'order by date desc").first()
        if user_exists:
            ...
            if datetime.now() - latest_entry > timedelta(days=3):
                ...
                message = client.messages.create(
                    body="Of course, if you don't want to hear from me anymore, just type 'unsubscribe' and I'll deactivate your account.", from_=twilio_phone, to=client_string)
            else:
                message = client.messages.create(
                    body=list_of_messages[randint(0, 2)], from_=twilio_phone, to=client_string)
        else:
            message = client.messages.create(
                body=list_of_messages[randint(0, 2)], from_=twilio_phone, to=client_string)
...
def user_setup(phone_num, name):
    ...
    client.messages.create(
        from_=twilio_phone, to=phone_num, body=first_paragraph)
    client.messages.create(
        from_=twilio_phone, to=phone_num, body=second_paragraph)
    client.messages.create(
        from_=twilio_phone, to=phone_num, body=third_paragraph)
    client.messages.create(
        from_=twilio_phone, to=phone_num, body=fourth_paragraph)
    client.messages.create(
        from_=twilio_phone, to=phone_num, body=fifth_paragraph)
